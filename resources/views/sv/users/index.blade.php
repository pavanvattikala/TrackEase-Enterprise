		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<div class="div-action pull pull-right" style="padding-bottom:20px;">
					<button class="btn btn-default button1" data-toggle="modal" data-target="#addUsersModalBtn"> <i class="glyphicon glyphicon-plus-sign"></i> Add User </button>
				</div> 		
				...
				<table class="table" id="manageUsersTable">
                    ...
                    @php
                        $users =  DB::table('users')->join('user_roles','user_roles.id',"=",'users.user_role')->select('user_roles.name as role_name','users.id','users.name','users.created_at','users.email')->where('users.id','>=',Auth::user()->user_role)->where('status',1)->get();
                       // dd($users);
                    @endphp
                    <tbody>
...
{{-- add brand --}}
<div class="modal fade" id="addUsersModalBtn" tabindex="-1" role="dialog">
	<div class="modal-dialog">
	  <div class="modal-content">
		  
		  <form class="form-horizontal" id="submitBrandForm" action="users/createuser" method="POST">
			  @csrf
			...
			<div class="modal-body">
				...
				<div class="form-group">
					...
					<div class="col-sm-8">
						<input type="text" class="form-control" id="email" placeholder="User Email" name="email" autocomplete="off">
					</div>
				</div>        	        
				<div class="form-group">
					<label for="userRole" class="col-sm-4 control-label">User Role: </label>
					@php
						$userRoles = DB::table('user_roles')->select('id','name')->where('id','>',Auth::user()->user_role)->where('status',1)->get();
					@endphp
					<div class="col-sm-8">
